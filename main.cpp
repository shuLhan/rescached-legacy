...
static int process_tcp_clients()
{
	int		idx		= 0;
	int		s		= 0;
	uint16_t	len		= 0;
	...
	while (client) {
		s = FD_ISSET(client->_d, &_srvr_tcp._readfds);
		if (0 == s) {
			goto next;
		}
...
static int process_udp_clients()
{
	int		idx		= 0;
	int		s		= 0;
...
int main(int argc, char *argv[])
{
	...
	while (_running_) {
		...
		if (FD_ISSET(_srvr_udp._d, &readfds)) {
			...
			process_udp_clients();
		} else if (FD_ISSET(_srvr_tcp._d, &readfds)) {
